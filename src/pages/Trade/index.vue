  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ defaultAddressList.userAddress }}</span>
        收货人：<span>{{ defaultAddressList.consignee }}</span>
        <span>{{ defaultAddressList.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      // 提交订单用户需要的数据
      let tradeInfo = {
        consignee: this.defaultAddressList.consignee,
        consigneeTel: this.defaultAddressList.phoneNum,
        deliveryAddress: this.defaultAddressList.userAddress,
        paymentWay: "ONLINE",
        orderComment: this.message,
        orderDetailList: this.detailArrayList,
      };
      //使用API直接发请求
      const result = await this.$API.reqSubmitOrder(
        this.tradeInfo.tradeNo,
        tradeInfo
      );
      if (result.code === 200) {
        alert("订单创建成功，自动跳转");
  ...
  computed: {
    ...
    // 订单默认地址，需要根据选中的默认地址计算
    defaultAddressList() {
      // 对地址列表进行遍历，筛选出 isDefault 属性为 1 的值
